...
- (CLLocationCoordinate2D*) visibleCoordinates {
    ...
    coordinates[0] = neCoord;
    coordinates[1] = swCoord;
    return coordinates;
     */
    return [self screenCoordinatesWithFlow:0];
}

- (CLLocationCoordinate2D*) screenCoordinatesWithFlow:(CGFloat)ratio {
    CLLocationCoordinate2D swCoordinate = [self convertPoint:CGPointMake(-self.frame.size.width*ratio, self.frame.size.height*(1+ratio)) toCoordinateFromView:self];
    CLLocationCoordinate2D neCoordinate = [self convertPoint:CGPointMake(self.frame.size.width*(1+ratio), -self.frame.size.height*ratio) toCoordinateFromView:self];
    CLLocationCoordinate2D *coordinates = malloc(sizeof(CLLocationCoordinate2D)*2);
    coordinates[0] = swCoordinate;
    coordinates[1] = neCoordinate;
    return coordinates;
}
